...
def test_specialize(debug=False):
    for t in substitutions:
        ...
        if debug:
            ...
            assert general.specialize(d) == special, "got " + str(general.specialize(d))      
...
def test_merge_specialization_maps(debug=False):
    for d1, d2, d in [
        ...
        ({'a':'b'}, None, None),
        (None, {'a':'b'}, None),
        ({'x':1, 'y':2}, {'x':1, 'z':3}, {'x':1, 'y':2, 'z':3}),
        ({'x':1, 'y':2}, {'x':1, 'y':3}, None),
        ({'x':1, 'y':2}, {'x':1, 'y':2, 'z':3}, {'x':1, 'y':2, 'z':3}),
        ({'x':1, 'y':2, 'z':3}, {'x':1, 'y':2}, {'x':1, 'y':2, 'z':3})]:
        if debug:
            print('Testing merging of dictionaries', d1, d2)
        dd = InferenceRule.merge_specialization_maps(d1,d2)
        assert dd == d, "got " + dd
...
rules = [
    ['(~p->~(q|T))', '(~(x|y)->~((z&(w->~z))|T))', [], [], {'p':'(x|y)', 'q':'(z&(w->~z))'}],
    ['(~p->~(q|T))', '(~(x|y)->((z&(w->~z))|T))', [], [], None],
    ['T', 'T', ['(~p->~(q|T))'], ['(~(x|y)->~((z&(w->~z))|T))'], {'p':'(x|y)', 'q':'(z&(w->~z))'}],
    ...
    ['p', 'r', ['(p->q)'],['(r->q)'], {'p':'r', 'q':'q'}],    
    ['p', 'r', ['(p->q)'],['(z->q)'], None],
    ['p', 'p1', ['(p->q)', '(p&p)'], ['(p1->r)','(p1&p1)'], {'p':'p1', 'q':'r'}],
    ['p', 'p1', ['(p->q)', '(p&p)'], ['(p1->(r&~z))','(p1&p1)'], {'p':'p1', 'q':'(r&~z)'}],
    ['p', '~T', ['(p->q)', '(p&p)'], ['(~T->(r&~z))','(~T&~T)'], {'p':'~T', 'q':'(r&~z)'}],
...
def test_is_line_valid(debug=False):
    ...
    if debug:
        print("\nChecking proof line vailidity in proof of", lemma, "using rules", {rule0, rule1})
    for i in range(len(z)):
        if debug:
...
def test_prove_specialization(debug=False):
    ...
    for instance_infix in [['(w|z)', '(z|w)'],
        ...
        assert instance_proof.is_valid(), instance_proof.offending_line()
...
def test_inline_proof_once(debug=False):
    ...
    assert lemma1_proof.is_valid()
    assert lemma2_proof.is_valid()
    ...
    if debug:
        print('Testing inline_proof_once (test 1). In main proof:\n',
              ...
              'with the the proof of following lemma proof:\n',
              str(lemma2_proof))    
    ...
    assert inlined_proof.is_valid(), inlined_proof.offending_line()
    ...
    for count in range(3):
        ...
        line_number = first_use_of_rule(proof, rule)
        if debug:
            print('Testing inline_proof_once (test ' + str(3+count) + '). In main proof:\n',
                  proof, "Replacing line", line_number,
                  'with the the proof of following lemma proof:\n',
                  str(lemma1_proof))    
...
def uses_of_rule(proof, rule):
    """ returns the number of lines in which the given proof uses the give rule
    """
    i=0
...
def test_inline_proof(debug=False):
    ...
    if debug:
        print("\nGot:", inlined_proof)
    assert inlined_proof.statement == proof.statement
    assert inlined_proof.rules == proof.rules.union(lemma2_proof.rules).difference({lemma2_proof.statement}), "Rule are: " + str(inlined_proof.rules)
    assert inlined_proof.is_valid(), inlined_proof.offending_line()
    ...
    if debug:
        print("\nGot:", inlined_proof)
    assert inlined_proof.statement == proof.statement
    assert inlined_proof.rules == proof.rules.union(lemma1_proof.rules).difference({lemma1_proof.statement})
    assert inlined_proof.is_valid(), inlined_proof.offending_line()
    ...
    assert inlined_lemma.rules == lemma1_proof.rules.union(lemma2_proof.rules).difference({lemma1_proof.statement})
    assert inlined_lemma.is_valid(), inlined_proof.offending_line()
    ...
    if debug:
        print("\nGot:", inlined_proof)
    assert inlined_proof.statement == proof.statement
    assert inlined_proof.rules == inlined_lemma.rules.union(inlined_proof.rules).difference({lemma2_proof.statement})
    assert inlined_proof.is_valid(), inlined_proof.offending_line()
