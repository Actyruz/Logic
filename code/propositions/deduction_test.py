""" (c) This file is part of the course
    ...
    File name: code/propositions/deduction_test.py """
...
def test_prove_corollary(debug=False):
    ...
    for g, r in [ (g1,r1), (g2, r2), (g3,r3) ]:
        if debug:
            print('Testing prove_corollary of', g, 'from proof of', pf.statement,
                  'using rule', r)
...
def test_combine_proofs(debug=False):
    ...
    for pp, pnq, h, r in [(pfp,pfnq,h1,NI),(pfp15,pfnq, h15, NI),
        ...
        if debug:
            print('Tetsing combine_proof of', h,'from', pp.statement, 'and', pnq.statement, 'using rule', r)
        ...
        assert pnpiq.is_valid(), pnpiq.offending_line()
...
Y3 = InferenceRule([], Formula.parse('(((x|y)|z)->(x|(y|z)))'))
Y4 = InferenceRule([], Formula.parse('((x|y)->(y|x))'))
Y5 = InferenceRule([], Formula.parse('(~p|p)'))
FUNNY_PROOF = Proof(
    InferenceRule([Formula.parse('((p|q)|r)')],
                  Formula.parse('((r|p)|q)')),
    {MP, I1, I2, Y3, Y4, Y5},
    [Proof.Line(Formula.parse('((p|q)|r)')),
     Proof.Line(Formula.parse('(((p|q)|r)->(p|(q|r)))'), Y3, []),
     Proof.Line(Formula.parse('(p|(q|r))'), MP, [0, 1]),
     Proof.Line(Formula.parse('((p|(q|r))->((q|r)|p))'), Y4, []),
     Proof.Line(Formula.parse('((q|r)|p)'), MP, [2, 3]),
     Proof.Line(Formula.parse('(((q|r)|p)->(q|(r|p)))'), Y3, []),
     Proof.Line(Formula.parse('(q|(r|p))'), MP, [4, 5]),
     Proof.Line(Formula.parse('((q|(r|p))->((r|p)|q))'), Y4, []),
     Proof.Line(Formula.parse('((r|p)|q)'), MP, [6, 7])])

# Used in remove_assumption
def prove_from_encoding(rule):
    """Given a rule [a1, a2, ... ak] => c, return a proof for it
    that uses as its rules MP as well as a single new rule that has no assumptions
    and whose conclusion is (((a1 -> (a2 -> ( ... -> (ak -> c))...))))"""
    ...
    lines.append(Proof.Line(newrule.conclusion, newrule, []))
...
def test_remove_assumption(debug=False):
    ...
    for oldp in [DISJUNCTION_COMMUTATIVITY_PROOF,
                 ...
                 FUNNY_PROOF]:
        p = oldp
        assert p.is_valid(), p.offending_line()
        while(True):
            rb = None
            for r in p.rules:
                if r != MP and r.assumptions != []:
                    rb = r
                    break
            if rb is None:
                break
            pr = prove_from_encoding(r)
            p = inline_proof(p, pr)
        assert p.is_valid(), p.offending_line()
        if debug:
        ...
        assert pp.rules.issubset(p.rules.union({MP,I0,I1,I2}))
        assert pp.is_valid(), pp.offending_line()
...
def test_prove_by_contradiction(debug=False):
    ...
    assert p.rules == {MP, I0, I1, I2, N, NI}
    assert p.is_valid(), p.offending_line()      
